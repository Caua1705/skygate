...
function renderPlanning() {
  ...
  const airportLabel = appData.airport?.city
    ? `Aeroporto de ${appData.airport.city}`
    : uiState.loading === 'airports' ? 'Conectando…' : 'Aeroporto de Fortaleza';

  /* ---- Quick shortcut categories ---- */
  const QUICK_CATS = [
    { key:'gates',     label:'Portões',    icon:'solar:routing-2-bold'    },
    { key:'restrooms', label:'Banheiros',  icon:'solar:bath-bold'         },
    { key:'services',  label:'Check-in',  icon:'solar:case-round-bold'   },
    { key:'food',      label:'Alimentação',icon:'solar:cup-hot-bold'      },
    { key:'shops',     label:'Lojas',      icon:'solar:bag-4-bold'        },
    { key:'access',    label:'Entradas',   icon:'solar:door-bold'         },
  ];

  return `
    <div class="sg-planning">
      <!-- APP BAR -->
      <header class="sg-appbar" role="banner">
        <div class="sg-appbar__brand">
          <img src="assets/logo.jpeg" alt="" class="sg-appbar__logo" aria-hidden="true">
          <div>
            <span class="sg-appbar__name">SkyGate</span>
            <span class="sg-appbar__loc">
              <span class="sg-dot sg-dot--${uiState.error ? 'error' : uiState.loading ? 'loading' : 'ok'}" aria-hidden="true"></span>
              ${esc(airportLabel)}
            </span>
          </div>
        </div>
        <button type="button" class="sg-icon-btn" id="help-btn" aria-label="Ajuda — como usar o SkyGate">
      ...
      <main class="sg-planning-main" id="planning-main">
        ...
        ` : uiState.error && !appData.floors.length ? `
          <div class="sg-fullstate sg-fullstate--error" role="alert">
            <iconify-icon icon="solar:danger-circle-bold" aria-hidden="true" style="font-size:36px;color:var(--red-600)"></iconify-icon>
            <p>${esc(uiState.error)}</p>
            <button type="button" class="sg-btn-primary" id="retry-btn" style="max-width:200px">Tentar novamente</button>
          </div>
        ` : `
          ...
          <div class="sg-planning-scroll">
            ...
            <!-- Section heading -->
            <div class="sg-planning-heading">
              <h1 class="sg-planning-title">Encontre seu caminho</h1>
              <p class="sg-planning-sub">Escolha onde você está e para onde deseja ir.</p>
            </div>

            <!-- Route form card -->
            <div class="sg-form-card" role="form" aria-label="Planejamento de rota">
              ...
              <!-- Origin + Destination cohesive input -->
              <div class="sg-route-input" role="group" aria-label="Origem e destino">
                <!-- Dotted connector line -->
                <div class="sg-route-input__line" aria-hidden="true"></div>
                ...
                <div class="sg-route-field">
                  ...
                  <button type="button"
                    class="sg-route-field__btn open-search"
                    data-kind="origin"
                    id="origin-btn"
                    aria-label="${oNode ? `Local de partida: ${esc(getPublicNodeLabel(oNode))}. Toque para mudar` : 'Selecionar local de partida'}"
                    aria-haspopup="dialog">
                    <span class="sg-route-field__label">Local de partida</span>
                    <span class="sg-route-field__value ${oNode ? '' : 'sg-route-field__value--ph'}">
                      ${oNode ? esc(getPublicNodeLabel(oNode)) : 'Buscar origem…'}
                    </span>
                    ${oNode ? `<span class="sg-route-field__sub">${esc(getPublicNodeSubtitle(oNode))}</span>` : ''}
                  </button>
                  ${oNode ? `<button type="button" class="sg-route-field__clear clear-loc" data-kind="origin" aria-label="Limpar local de partida" id="clear-origin">
                    <iconify-icon icon="solar:close-circle-bold" aria-hidden="true"></iconify-icon>
                  </button>` : ''}
                </div>
                ...
                <div class="sg-route-field">
                  ...
                  <button type="button"
                    ...
                    <span class="sg-route-field__value ${dNode ? '' : 'sg-route-field__value--ph'}">
                      ${dNode ? esc(getPublicNodeLabel(dNode)) : 'Buscar destino…'}
                    </span>
                    ${dNode ? `<span class="sg-route-field__sub">${esc(getPublicNodeSubtitle(dNode))}</span>` : ''}
                  </button>
                ...
                <!-- Swap button -->
                <button type="button" class="sg-swap-btn" id="swap-btn"
              ...
              <div class="sg-accessible-row">
                <label class="sg-accessible-label" for="accessible-toggle">
                  <iconify-icon icon="solar:accessibility-bold" aria-hidden="true" class="sg-accessible-icon ${isAccessible ? 'is-active' : ''}"></iconify-icon>
                  <div class="sg-accessible-text">
                    <span class="sg-accessible-title">Preciso de rota acessível</span>
                    <span class="sg-accessible-desc">Usa elevadores e evita escadas e escadas rolantes.</span>
                  </div>
                </label>
                <button type="button"
                  ...
                  aria-label="Rota acessível — usa elevadores, evita escadas">
              ...
              <div class="sg-form-actions">
                ...
                ${hint ? `<p class="sg-form-hint ${same ? 'sg-form-hint--warn' : ''}" role="status" aria-live="polite">${esc(hint)}</p>` : ''}
              </div>
            </div>

            <!-- Quick category shortcuts -->
            <section class="sg-quick-section" aria-label="Destinos rápidos">
              <h2 class="sg-quick-title">Acessos rápidos</h2>
              <div class="sg-quick-grid" role="list">
                ${QUICK_CATS.map(cat => `
                  <button type="button"
                    class="sg-quick-item"
                    ...
                    role="listitem">
                    <span class="sg-quick-item__icon" aria-hidden="true">
                      <iconify-icon icon="${cat.icon}"></iconify-icon>
                    </span>
                    <span class="sg-quick-item__label">${esc(cat.label)}</span>
                  </button>
                `).join('')}
              </div>
            </section>
...
function bindEvents() {
  ...
  document.querySelectorAll('.sg-quick-item').forEach(btn =>
...
function toggleAccessibleRoute() {
  ...
  if (toggleEl) {
    ...
    const icon = toggleEl.previousElementSibling?.querySelector('iconify-icon');
